...
RSpec.describe "Subscription Request" do
	describe "Customer Subscriptions" do
		context "when succesful" do
			it "returns all subscriptions active or inactive for a customer" do
				...
				data[:data].each do |customer_subscription|
					...
					expect(customer_subscription[:attributes].keys).to match([:customer_id, :status, :subscription])
		...
		context "when unsuccesful" do
			it "returns an error when no customer with that ID is found" do
				subs = create_list(:subscription, 5)
				customer_1 = create(:customer)
				customer_2 = create(:customer)
				CustomerSubscription.create(customer: customer_1, subscription: subs[0])
				...
				get api_v1_customer_subscriptions_path(7)
	...
	describe "create subscription" do
		context "when successful" do
			it "creates a subscription to the customer" do
				...
				post api_v1_customer_subscriptions_path(customer), headers: headers, params: JSON.generate(subscription: subscription_params)
				...
				expect(data[:data][:attributes].keys).to match([:title, :price, :status, :frequency])
		...
		context "when unsuccessful" do
			it "user doesn't exist" do
				...
				subscription_params = { id: sub.id }

				headers = { "Content-Type" => "application/json"}

				post api_v1_customer_subscriptions_path(7), headers: headers, params: JSON.generate(subscription: subscription_params)
			...
			it "no subscription sent" do
				...
				post api_v1_customer_subscriptions_path(customer), headers: headers
				...
				data[:errors].each do |error|
					...
					expect(error[:title]).to be_a String
					expect(error[:title]).to eq("param is missing or the value is empty: subscription")
				end
			end
	...
	describe "update the status (cancel) of a customer subscription" do
		context "when successful" do
			it "updates the status of a customer subscription to inactive" do
				...
				patch api_v1_customer_subscription_path(Customer.first, Subscription.first), headers: headers, params: JSON.generate(subscription: body)
				...
				expect(data[:data][:type]).to be_a String
				expect(data[:data][:attributes]).to be_a Hash
				expect(data[:data][:attributes].keys).to match([:customer_id, :status, :subscription])
